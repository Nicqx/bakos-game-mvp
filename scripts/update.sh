...
IMAGE="bakos-game:${REV}-${NODE_ARCH}"
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT INT TERM
sed "s|image: bakos-game:1.0.0|image: $IMAGE|" k8s/bakos-game.yaml > "$TMP/resources.yaml"
...
STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/nicqx-apps/$TARGET/bakos-game"
mkdir -p -m 700 "$STATE_DIR"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
$KUBECTL get deployment/bakos-game service/bakos-game-service -n "$NAMESPACE" -o yaml \
...
$KUBECTL rollout status deployment/bakos-game -n "$NAMESPACE" --timeout=180s
echo "Ready: Deployment/bakos-game ($IMAGE)"
